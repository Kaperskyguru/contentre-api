...
import { Brief, MutationUpdateBriefArgs } from '@modules-types'
import { Context } from '@types'
import { ApolloError } from 'apollo-server-errors'

export default async (
  _parent: unknown,
  { id, input }: MutationUpdateBriefArgs,
  { user, sentryId, prisma, ipAddress, requestURL }: Context & Required<Context>
): Promise<Brief> => {
  logMutation('updateBrief %o', {
    input,
    user,
    ipAddress,
    requestURL
  ...
  try {
    ...
    // Finally update the brief.
    return await prisma.content.update({
      where: { id },
      data
    })
  } catch (e) {
    logError('updateBrief %o', {
      input,
      user,
      ipAddress,
      requestURL,
