...
export default async (
  ...
  logQuery('getGetContentStat %o', user)
  try {
    ...
    const lastYear: AmountGroupQueryResult[] = await prisma.$queryRawUnsafe(
      `
        ...
        CASE when COUNT(c."id")::INT IS NULL THEN 0 ELSE COUNT(c."id")::INT END amount
        FROM  (
        ...
        WHERE
          c."notebookId" IS NULL AND c."class" = 'ARTICLE'
          AND (c."userId" = $1 or c."userId" is null )
    ...
    const currentYear: AmountGroupQueryResult[] = await prisma.$queryRawUnsafe(
      ...
      where c."notebookId" IS NULL AND c."class" = 'ARTICLE'
      AND (c."userId" = $1 or c."userId" is null ) 
      GROUP BY 1;
    ...
    logError('getGetContentStat %o', e)
